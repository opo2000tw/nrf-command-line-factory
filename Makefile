...
package: dist
	...
	zip -q $(DIST)/nrf-factory-$(VERSION)-windows-amd64.zip $(DIST)/$(BIN_WIN) 3rd/nrfutil.exe CHANGELOG.md docs/INSTALL-AND-RUN.md
	zip -q $(DIST)/nrf-factory-$(VERSION)-darwin-arm64.zip  $(DIST)/$(BIN_MAC_ARM) 3rd/nrfutil CHANGELOG.md docs/INSTALL-AND-RUN.md
